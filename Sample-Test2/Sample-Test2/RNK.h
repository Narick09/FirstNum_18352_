...
public:
	...
	//---------------------------------
//	Nucleotide TmpGeterNuc(size_t i) const;													//for debag
	size_t capacity() const { return this->physSize; }
	//---------------------------------
	~RNK();
};

//переделать split
//подумать над каждой функцией и над ошибками, которые она может вызвать, ну и распихать все по юнит тестам.
//чем отличается unordered_map от простого map?
//